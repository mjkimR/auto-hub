...
Environment variables PROJECT_ID, REGION, DATABASE_URL, DB_CONNECTION_NAME, SA_NAME can also be used.
EOF
  exit 0
}
...
# Helper function to create secret if it does not exist, or add version if empty
...
# 2. auto-hub-app-secret (32 bytes hex)
APP_SECRET_VAL=$(openssl rand -hex 32)
...
echo "    App Secret Key (for UI login):"
