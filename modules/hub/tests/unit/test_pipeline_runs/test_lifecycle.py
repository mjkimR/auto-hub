...
def create_mock_run(
    ...
    run.linear_issue_id = uuid4()
    run.linear_issue_identifier = "HUB-10"
    run.linear_issue_snapshot = {
        "id": str(run.linear_issue_id),
        "identifier": "HUB-10",
        "title": "Test Issue",
        "priority": 1,
        "state_type": "started",
        ...
        "url": "https://linear.app/test/HUB-10",
    }
    run.state = state
    run.pause_reason = None
    run.branch = "codex/hub-10"
    run.pull_number = pull_number
    run.pull_url = pull_url
    run.revision = 1
